...
//   time  : SysTick（内核定时器）
//   EEPROM: 无硬件，注释留 DataFlash 模拟参考（CH32V003 用户区 1KB）
...
void fe_port_timer0_init(void) {
    // SysTick: 48MHz / 1000 = 48000 周期
    SYST_RVR = 48000u - 1u;
    SYST_CVR = 0;
    // TICKINT(bit1) 必须置位: 旧实现只置 CLKSOURCE|ENABLE 且无 SysTick_Handler,
    // s_tick_ms 永不自增 → 系统时间恒 0。
...
u32 fe_port_time_now(void) {
    return s_epoch_base + s_tick_ms / 1000UL;
}

void fe_port_time_set(u32 epoch) {
    // 存 epoch - uptime, 使 now = epoch + 相对秒(避免二次计数)。
...
int fe_port_gpio_set_mode(u8 pin, const char *mode) {
    ...
    // 使能端口时钟（简化：直接置位）
    // 引脚映射覆盖 GPIOA(pin 0-7)/GPIOB(pin 8-15)/GPIOC(pin 16-19, 契约 D0-D13+A0-A5)，
...
void fe_port_chip_info(char *out, u16 outlen) {
    fe_snprintf(out, outlen,
                "{\"chip\":\"CH32V003\",\"arch\":\"RISC-V RV32EC\","
